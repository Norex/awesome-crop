...
imageEdit = {
    ...
    refreshSelection : function(postid) {
        var data = {
            action: 'awesomecrop_aspect_ratio',
            field_key: window.top.field_key,
            post_type: window.top.post_type
        };

        $.post(ajaxurl, data, function(res) {
            if (res['enabled'] == 1) {
                imageEdit.enabled = true;
                $('#imgedit-crop-width-' + postid).val(res['aspect_ratio_width']);
                $('#imgedit-crop-height-' + postid).val(res['aspect_ratio_height']);

                imageEdit.setDisabled($('#imgedit-crop-width-' + postid).parents('p'));
                imageEdit.setDisabled($('#imgedit-crop-width-' + postid));
                imageEdit.setDisabled($('#imgedit-crop-height-' + postid));
                $('#imgedit-crop-width-' + postid).attr('disabled', 'disabled');
                $('#imgedit-crop-height-' + postid).attr('disabled', 'disabled');


                $('#imgedit-sel-width-' + postid).val(res['width']);
                $('#imgedit-sel-height-' + postid).val(res['height']);

                imageEdit.setDisabled($('#imgedit-crop-sel-' + postid));
                imageEdit.setDisabled($('#imgedit-sel-width-' + postid));
                imageEdit.setDisabled($('#imgedit-sel-height-' + postid));
                $('#imgedit-sel-width-' + postid).attr('disabled', 'disabled');
                $('#imgedit-sel-height-' + postid).attr('disabled', 'disabled');

                imageEdit.setDisabled($('.imgedit-crop', '#imgedit-panel-' + postid), 1); // Show the crop button.

                $('#imgedit-sel-width-' + postid).keyup();
                $('#imgedit-sel-height-' + postid).keyup();
            }
        }, 'json');
    },
        ...
        t.iasapi = $(image).imgAreaSelect({
            ...
            onInit: function(img, c) {
                parent.children().mousedown(function(e){
                    ...
                    // Only have shift key cropping if this is an image with locked aspect ratios.
                    if (imageEdit.enabled == true) {
    ...
    setNumSelection : function(postid) {
        ...
        var aspectWidth = $('#imgedit-crop-width-' + postid).val();
        var aspectHeight = $('#imgedit-crop-height-' + postid).val();
        ...
        if ( x && y && ( sel = ias.getSelection() ) ) {
            ...
            if (x2 > imgw && y2 > imgh) {
                x1 = 0;
                x2 = imgw;
                elY.val(Math.round(((x2 / aspectWidth) * aspectHeight) / sizer)); // Adjust aspect ratio if it doesn't fit.

                y1 = 0;
                y2 = imgh;
                elX.val( Math.round(((y2 / aspectHeight) * aspectWidth) / sizer) ); // Adjust aspect ratio if it doesn't fit.
